...
export default function Home() {
  return (
    <div className="bg-[radial-gradient(circle_at_top_left,_#749B8F60_0%,_transparent_95%)] bg-white font-sans text-gray-700 min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-4 py-8 md:px-6 md:py-24">
        <div className="grid md:grid-cols-2 gap-6 md:gap-12 items-center">
          ...
          <div className="order-1 md:order-1 space-y-3 md:space-y-4">
            ...
            <h2 className="font-bold text-lg md:text-2xl text-[#749B8F]">
      ...
      {/* Services Section */}
      <section className="max-w-6xl mx-auto px-4 py-12 md:px-6 md:py-20">
        <h2 className="font-sans text-xl md:text-4xl font-bold text-gray-800 text-center mb-8 md:mb-16">
          ¿En qué te ayudaré?
        </h2>
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-10">
      ...
      {/* Grid Buttons CTA */}
      <section className="max-w-5xl mx-auto px-4 py-12 md:px-6 md:py-24">
      ...
      <section className="max-w-4xl mx-auto px-4 py-10 md:px-6 md:py-20">
        <div className="flex flex-col items-center">
          ...
          <div className="text-center max-w-xl">
            <p className="font-serif italic text-sm md:text-2xl text-gray-500 leading-relaxed">
              "Desenredar el ovillo de la mente no es borrar el pasado, sino aprender a caminar entre sus hilos con claridad y compasión."
            </p>
            <div className="w-10 h-0.5 bg-[#749B8F]/20 mx-auto mt-4 md:mt-6 rounded-full"></div>
          </div>
        </div>
      </section>
      ...
      <footer className="w-full bg-[#4A635D] text-white pt-10 pb-6 md:pt-20 md:pb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-12 mt-8 md:mt-16 pt-4 md:pt-8 border-t border-white/10 text-center text-[10px] md:text-xs opacity-60">
...
function ServiceItem({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="flex flex-col items-center text-center group cursor-default">
      <div className="w-14 h-14 md:w-20 md:h-20 bg-[#749B8F] rounded-full flex items-center justify-center mb-3 md:mb-6 shadow-md transform transition-transform group-hover:scale-110 group-hover:bg-[#63857a]">
        {icon}
      </div>
      <h3 className="font-sans text-xs md:text-base font-bold text-gray-700 leading-tight uppercase tracking-wider">
        {title}
      </h3>
    </div>
  );
...
function CTAButton({ text, href = "#" }: { text: string; href?: string }) {
  return (
    <Link
      href={href}
      className="w-full bg-[#749B8F] text-white py-4 md:py-8 px-4 md:px-6 rounded-lg text-center font-sans font-bold text-sm md:text-xl shadow-lg hover:bg-[#63857a] hover:shadow-xl transition-all duration-300"
    >
      {text}
    </Link>
  );
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="text-xs md:text-sm opacity-80 hover:opacity-100 hover:translate-x-1 transition-all">
      {children}
    </Link>
  );
}
